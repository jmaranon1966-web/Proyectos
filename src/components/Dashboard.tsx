...
import { Activity, AlertTriangle, CheckCircle, Clock, Plus, X, Trash2, AlertOctagon, Filter, Eye, EyeOff, Zap, DollarSign } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Power Calculations
  const totalPower = projects.reduce((acc, p) => acc + (p.capacityKW || 0), 0);
  const completedPower = projects.filter(p => p.status === 'Completado').reduce((acc, p) => acc + (p.capacityKW || 0), 0);
      ...
      <div key={project.id} className="relative flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors group mb-3 last:mb-0 border border-slate-100">
        <div className="flex-1">
            ...
            <div className="text-sm text-slate-500 flex items-center gap-2 flex-wrap">
                ...
                {(project.capacityKW || project.budget) && (
                    <>
                        <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                        <span className="text-xs font-medium text-slate-600 flex items-center">
                            {project.capacityKW ? <><Zap className="w-3 h-3 mr-0.5 text-yellow-500" /> {project.capacityKW} kWp</> : ''}
                            {project.capacityKW && project.budget ? <span className="mx-1">|</span> : ''}
                            {project.budget ? <><DollarSign className="w-3 h-3 mr-0.5 text-emerald-500" /> {project.budget.toLocaleString()}</> : ''}
                        </span>
                    </>
                )}
            </div>
    ...
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
             <div className="flex justify-between items-start">
                <div>
                <p className="text-sm font-medium text-slate-500">Potencia (kWp)</p>
                <div className="flex items-end space-x-1 mt-1">
                    <h3 className="text-xl font-bold text-slate-900">{completedPower.toLocaleString()}</h3>
                    <span className="text-xs text-slate-400 mb-1">/ {totalPower.toLocaleString()} Total</span>
                </div>
                <p className="text-[10px] text-slate-400">Instalada vs Planificada</p>
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Potencia (kWp)</label>
                            <input 
                                type="number"
                                min="0"
                                placeholder="0"
