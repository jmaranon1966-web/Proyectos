...
// NOTE: Roles are now dynamic in App state, removed static const PROJECT_ROLES
...
export interface User {
  ...
  department: string; 
  avatar: string;
  accessCode: string; // NEW: System generated ID for login/audit
  isActive: boolean;
}
...
export interface AuditLogEntry {
    id: string;
    timestamp: string;
    action: string; // e.g., "Generar Reporte", "Crear Usuario"
    details: string; // e.g., "Reporte PDF descargado"
    user: string; // Name of the user who performed action (or 'Sistema')
